...
Host binary path: `install.sh | sh -s -- --yes --runtime host`, then `cd ~/beampipe && beampipe start`. Slurm keys stay under `deploy/ssh/credentials/<slot>/`; use `--acl` so uid 10001 can read them. See [deploy/ssh/README.md](https://github.com/jbwod/beampipe-core-v2/blob/main/deploy/ssh/README.md).
